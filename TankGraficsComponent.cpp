...
void TankGraficsComponent::update(GameObject* gameObject)
{
	...
	D2D1::Matrix3x2F translationMatrix = D2D1::Matrix3x2F::Translation(D2D1::SizeF(transSize.x - m_pTankBody->getWidth() / 2, transSize.y - m_pTankBody->getHeight() / 2));
...
int TankGraficsComponent::getBodyWidth() const
{
	return m_pTankBody->getWidth();
}

int TankGraficsComponent::getBodyHeight() const
{
	return m_pTankBody->getHeight();
}
